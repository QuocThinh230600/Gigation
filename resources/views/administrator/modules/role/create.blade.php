...
@section('title', title_module('role','create'))
...
@section('content')
    <form action="{{ route('admin.role.store') }}" method="POST" enctype="multipart/form-data" class="formAjax">
        @csrf
        @method('POST')
                                ...
                                <ul class="list-permission-child">
                                    @foreach ($permissions as $permission)
                                        <li>
                                            <div class="form-check">
                                                <label class="form-check-label">
                                                    <input type="checkbox" name="permission[]" class="form-check-input-styled permission" value="{{ $permission['value'] }}" {{ checkedRole (old('permission'), $permission['value']) }} /> {{ $permission['name'] }}
                                                </label>
                                            </div>
                                        </li>
                                    @endforeach
