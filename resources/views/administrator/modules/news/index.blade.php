...
@section('title', title_module('news','index'))

@can('news_create')
    @section('create',route('admin.news.create'))
@endcan
...
@section('content')
    <x-card title="action.info" table="true">
        <table class="table table-hover datatable-colvis-basic" url="{{ route('admin.ajax.newsDataTables') }}">
            <thead>
                <tr>
                    <th>{{ label('element.table_id') }}</th>
                    <th>{{ label('news.image') }}</th>
                    <th>{{ label('news.title') }}</th>
                    <th>{{ label('news.category') }}</th>
                    <th>{{ label('element.status') }}</th>
                    <th>{{ label('element.featured') }}</th>
                    @canany(['news_edit', 'news_destroy'])
