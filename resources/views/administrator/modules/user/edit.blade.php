...
@section('title', title_module('user', 'edit'))
...
@section('content')
    <form action="{{ route('admin.user.update', ['user' => $user->uuid]) }}" method="POST" enctype="multipart/form-data" class="formAjax">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <x-card title="action.info" id="forms-target-left">
                    <x-text label="user.email" type="email" disabled="disabled" name="email" placeholder="user.email" required="required">
